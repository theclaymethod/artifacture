...
```json
{
  ...
  "libraries_needed": [],
  "diagram_sources": [],
  "notes": "Used the hero-number component. Headline copy passed the $unslop sister skill."
}
```
...
6. **Stitch.**
   ...
   - If any fragment has `libraries_needed` containing `"mermaid"`, add the Mermaid CDN import + the Mono-Industrial init script + the diagram-render loop that reads escaped text from hidden `.ve-diagram__source` elements and renders into the matching `.ve-diagram__canvas`. **Dedup.**
