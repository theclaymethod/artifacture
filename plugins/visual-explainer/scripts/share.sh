...
SHAREHTML_CONFIG="${HOME}/.config/visual-explainer/share.json"
if [ -n "${VE_SHAREHTML_URL:-}" ] || [ -f "$SHAREHTML_CONFIG" ]; then
    if ! command -v sharehtml >/dev/null 2>&1; then
        echo -e "${RED}Error: sharehtml CLI not found${NC}" >&2
        echo "Install sharehtml first, then retry:" >&2
        echo "  bun install -g sharehtml" >&2
        echo "Or use your sharehtml repo checkout's CLI on PATH." >&2
        exit 1
    fi

    echo -e "${CYAN}Sharing $(basename "$HTML_FILE") via sharehtml...${NC}" >&2
    echo -e "${CYAN}Share mode: private team link, stable update-in-place URL${NC}" >&2
    sharehtml deploy "$HTML_FILE"
    exit $?
fi
